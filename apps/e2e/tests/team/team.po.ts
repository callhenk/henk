...
export class TeamPageObject {
  ...
  async selectFirstBusiness() {
    ...
    const businessCards = this.page.locator('[class*="glass-panel"][class*="cursor-pointer"]');

    // Check if any business cards exist
    const count = await businessCards.count();

    if (count === 0) {
      throw new Error('No businesses found. User needs to create a business first.');
    }
    ...
    await expect(this.page.getByText(/Team Members -/)).toBeVisible({ timeout: 10000 });
  }
  ...
  async selectBusinessByName(name: string) {
    await this.page.waitForLoadState('networkidle');
    const businessCard = this.page.locator(`[class*="glass-panel"]:has-text("${name}")`);
  ...
  async inviteMember(params: { email: string; role?: 'owner' | 'admin' | 'member' | 'viewer' }) {
    ...
    if (params.role && params.role !== 'member') {
      ...
      await this.page.getByRole('option', { name: params.role, exact: true }).click();
    }

    // Submit invitation
    await this.page.getByRole('button', { name: /Invite Member/i }).last().click();

    // Wait for dialog to close
    await expect(this.page.getByText('Invite Team Member')).not.toBeVisible({ timeout: 5000 });
  }
  ...
  async getMemberCount(status?: 'all' | 'active' | 'invited') {
    if (status === 'active') {
      const activeTab = await this.page.getByRole('tab', { name: /Active/ }).textContent();
      const match = activeTab?.match(/\((\d+)\)/);
      return match ? parseInt(match[1]) : 0;
    } else if (status === 'invited') {
      const invitedTab = await this.page.getByRole('tab', { name: /Invited/ }).textContent();
      const match = invitedTab?.match(/\((\d+)\)/);
      return match ? parseInt(match[1]) : 0;
    } else {
      const allTab = await this.page.getByRole('tab', { name: /All Members/ }).textContent();
      const match = allTab?.match(/\((\d+)\)/);
      return match ? parseInt(match[1]) : 0;
    }
  }

  async updateMemberRole(memberEmail: string, newRole: 'owner' | 'admin' | 'member' | 'viewer') {
