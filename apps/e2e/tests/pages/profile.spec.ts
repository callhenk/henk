...
test.describe('Profile Page Tests', () => {
  ...
  test('can view profile information', async ({ page }) => {
    ...
    if (await profileLink.isVisible({ timeout: 2000 })) {
      ...
      const emailField = page.locator('[data-test="account-email-form-email-input"]');
  ...
  test('can open profile menu', async ({ page }) => {
    ...
    if (await profileButton.isVisible({ timeout: 2000 })) {
      ...
      const menuVisible = await page.locator('text=Sign Out, text=Settings, text=Profile').first().isVisible({ timeout: 1000 }).catch(() => false);
