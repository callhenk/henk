...
create policy "account_image_bucket_access"
on "storage"."buckets"
as permissive
for select
to public
using (((id = 'account_image'::text) AND (auth.role() = 'authenticated'::text)));


create policy "agent_assets_bucket_access"
on "storage"."buckets"
as permissive
for select
to public
using (((id = 'agent_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "campaign_assets_bucket_access"
on "storage"."buckets"
as permissive
for select
to public
using (((id = 'campaign_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "knowledge_base_bucket_access"
on "storage"."buckets"
as permissive
for select
to public
using (((id = 'knowledge_base'::text) AND (auth.role() = 'authenticated'::text)));


create policy "workflow_assets_bucket_access"
on "storage"."buckets"
as permissive
for select
to public
using (((id = 'workflow_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "Give users access to generated folder delete"
on "storage"."objects"
as permissive
for delete
to authenticated
using (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'generated'::text) AND (auth.role() = 'authenticated'::text)));
...
create policy "Give users access to generated folder update"
on "storage"."objects"
as permissive
for update
to authenticated
using (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'generated'::text) AND (auth.role() = 'authenticated'::text)))
with check (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'generated'::text) AND (auth.role() = 'authenticated'::text)));


create policy "Give users access to generated folder upload"
on "storage"."objects"
as permissive
for insert
to authenticated
...
create policy "Give users access to samples folder delete"
on "storage"."objects"
as permissive
for delete
to authenticated
using (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'samples'::text) AND (auth.role() = 'authenticated'::text)));


create policy "Give users access to samples folder read"
on "storage"."objects"
as permissive
for select
to authenticated
...
create policy "Give users access to samples folder update"
on "storage"."objects"
as permissive
for update
to authenticated
using (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'samples'::text) AND (auth.role() = 'authenticated'::text)))
with check (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'samples'::text) AND (auth.role() = 'authenticated'::text)));


create policy "Give users access to samples folder upload"
on "storage"."objects"
as permissive
for insert
to authenticated
...
for select
to public
using (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'private'::text) AND (auth.role() = 'authenticated'::text)));
...
for insert
to public
with check (((bucket_id = 'audio'::text) AND ((storage.foldername(name))[1] = 'private'::text) AND (auth.role() = 'authenticated'::text)));
...
create policy "Give users authenticated access to folder 1jgvrq_3"
on "storage"."objects"
as permissive
for delete
to public
...
create policy "Secure profile image access"
on "storage"."objects"
as permissive
for select
to public
using (((bucket_id = 'account_image'::text) AND (((auth.uid())::text = split_part(storage.filename(name), '.'::text, 1)) OR (auth.role() = 'authenticated'::text))));
...
for select
to public
using (((bucket_id = 'account_image'::text) AND ((auth.uid())::text = split_part(name, '.'::text, 1))));
...
for delete
to public
using (((bucket_id = 'account_image'::text) AND ((auth.uid())::text = split_part(name, '.'::text, 1)) AND (auth.role() = 'authenticated'::text)));
...
for delete
to public
using (((bucket_id = 'account_image'::text) AND (auth.role() = 'authenticated'::text) AND ((auth.uid())::text = split_part(storage.filename(name), '.'::text, 1))));
...
for insert
to public
with check (((bucket_id = 'account_image'::text) AND ((auth.uid())::text = split_part(name, '.'::text, 1)) AND (auth.role() = 'authenticated'::text)));
...
for insert
to public
with check (((bucket_id = 'account_image'::text) AND (auth.role() = 'authenticated'::text) AND ((auth.uid())::text = split_part(storage.filename(name), '.'::text, 1))));
...
for all
to public
using (((bucket_id = 'agent_assets'::text) AND ((storage.foldername(name))[1] IN ( SELECT (a.id)::text AS id
   FROM (agents a
     JOIN team_members tm ON ((a.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))))
with check (((bucket_id = 'agent_assets'::text) AND ((storage.foldername(name))[1] IN ( SELECT (a.id)::text AS id
   FROM (agents a
     JOIN team_members tm ON ((a.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))));


create policy "agent_assets_delete"
on "storage"."objects"
as permissive
for delete
to public
using (((bucket_id = 'agent_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "agent_assets_insert"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'agent_assets'::text) AND (auth.role() = 'authenticated'::text)));
...
create policy "agent_assets_update"
on "storage"."objects"
as permissive
for update
to public
...
create policy "campaign_assets"
on "storage"."objects"
as permissive
for all
to public
using (((bucket_id = 'campaign_assets'::text) AND ((storage.foldername(name))[1] IN ( SELECT (c.id)::text AS id
   FROM (campaigns c
     JOIN team_members tm ON ((c.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))))
with check (((bucket_id = 'campaign_assets'::text) AND ((storage.foldername(name))[1] IN ( SELECT (c.id)::text AS id
   FROM (campaigns c
     JOIN team_members tm ON ((c.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))));
...
for delete
to public
using (((bucket_id = 'campaign_assets'::text) AND (auth.role() = 'authenticated'::text)));
...
for insert
to public
with check (((bucket_id = 'campaign_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "campaign_assets_select"
on "storage"."objects"
as permissive
for select
to public
...
create policy "knowledge_base"
on "storage"."objects"
as permissive
for all
to public
using (((bucket_id = 'knowledge_base'::text) AND ((storage.foldername(name))[1] IN ( SELECT (a.id)::text AS id
   FROM (agents a
     JOIN team_members tm ON ((a.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))))
with check (((bucket_id = 'knowledge_base'::text) AND ((storage.foldername(name))[1] IN ( SELECT (a.id)::text AS id
   FROM (agents a
     JOIN team_members tm ON ((a.business_id = tm.business_id)))
  WHERE ((tm.user_id = auth.uid()) AND (tm.status = 'active'::team_member_status))))));
...
for delete
to public
using (((bucket_id = 'knowledge_base'::text) AND (auth.role() = 'authenticated'::text)));
...
for insert
to public
with check (((bucket_id = 'knowledge_base'::text) AND (auth.role() = 'authenticated'::text)));
...
create policy "workflow_assets_delete"
on "storage"."objects"
as permissive
for delete
to public
using (((bucket_id = 'workflow_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "workflow_assets_insert"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'workflow_assets'::text) AND (auth.role() = 'authenticated'::text)));


create policy "workflow_assets_select"
on "storage"."objects"
as permissive
for select
to public
...
create policy "workflow_assets_update"
on "storage"."objects"
as permissive
for update
to public
