    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          data-minimized={!open}
          className={cn(
            'text-sidebar-foreground has-data-[variant=inset]:bg-sidebar group flex min-h-svh w-full',
            className,
          )}
          ref={ref}
          {...props}
  ...
  return (
    <main
      className={cn(
        ...
        'peer-data-[variant=inset]:min-h-[calc(100svh-(--spacing(4)))] md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 md:peer-data-[variant=inset]:rounded-xl',
        className,
      )}
      {...props}
    />
...
> = ({ className, asChild = false, ...props }) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-muted-foreground ring-sidebar-ring outline-hidden flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
                            ...
                            >
                              <Link
                                className={cn('flex items-center', {
                                  'gap-0! mx-auto w-full [&>svg]:flex-1': !open,
                                })}
                                href={path}
                              >
                                {child.Icon}
                                    ...
                                    >
                                      {children.map((child) => {
                                        ...
                                        const linkClassName = cn(
                                          'flex items-center',
                                          {
                                            'gap-0! mx-auto w-full [&>svg]:flex-1':
                                              !open,
                                          },
                                        );
